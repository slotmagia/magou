...
<script lang="ts" setup>
  ...
  import { getTenantUserList, createTenantUser, updateTenantUser, deleteTenantUser, resetTenantUserPassword } from '@/api/system/user';
        ...
        try {
          if (formParams.value.id > 0) {
            ...
            const params = {
              ...
              realName: formParams.value.realName,
              nickname: formParams.value.nickname,
              password: btoa(formParams.value.password || '123456'), // Base64编码
              gender: formParams.value.gender,
              birthday: formParams.value.birthday,
              deptId: formParams.value.deptId,
              position: formParams.value.position,
              status: formParams.value.status || 1,
              remark: formParams.value.remark,
            };
            await createTenantUser(params);
          }
