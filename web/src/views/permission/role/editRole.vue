...
<script lang="ts" setup>
  ...
  import { Edit, getRoleList, createRole, updateRole } from '@/api/system/role';
        ...
        try {
          if (formValue.value.id > 0) {
            ...
            const params = {
              ...
              code: formValue.value.key,
              description: formValue.value.remark,
              status: formValue.value.status,
              sort: formValue.value.sort,
              dataScope: formValue.value.dataScope || 1,
  ...
  function loadDataList() {
    ...
    getRoleList({ pageSize: 100, page: 1 }).then((res) => {
      ...
      if (res.data && res.data.list) {
        rawRoleOption.value = res.data.list;
      } else {
