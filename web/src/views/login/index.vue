...
<script lang="ts" setup>
  ...
  const activeModule = shallowRef<LoginModule>({
    key: 'login',
    label: '账号登录',
    component: markRaw(LoginFrom),
  });

  const modules: LoginModule[] = [
    { key: 'login', label: '账号登录', component: markRaw(LoginFrom) },
    // { key: 'register', label: '注册账号', component: markRaw(RegisterFrom) },
    // { key: 'reset-pwd', label: '重置密码', component: markRaw(ResetPwd) },
    // { key: 'bind-wechat', label: '绑定微信', component: markRaw(BindWechat) }
  ];
  ...
  onMounted(() => {
    //是否开放注册 - 默认不开放
    const registerEnabled = userStore.loginConfig?.loginRegisterSwitch === 1;
    if (registerEnabled) {
      const findItem = modules.find((item) => item.key === 'register');
      if (!findItem) {
        modules.push({ key: 'register', label: '注册账号', component: markRaw(RegisterFrom) });
      }
    }
