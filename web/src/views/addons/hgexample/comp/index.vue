...
<script lang="ts" setup>
  ...
  const components = {
    formBasic: markRaw(FormBasic),
    formUseForm: markRaw(FormUseForm),
    modal: markRaw(Modal),
    drag: markRaw(Drag),
    directive: markRaw(Directive),
    resultSuccess: markRaw(ResultSuccess),
    resultFail: markRaw(ResultFail),
    resultInfo: markRaw(ResultInfo),
    exception403: markRaw(Exception403),
    exception404: markRaw(Exception404),
    exception500: markRaw(Exception500),
    visPPchart: markRaw(VisPPchart),
    visEcharts: markRaw(VisEcharts),
    visMadeAPie: markRaw(VisMadeAPie),
    textPinyin: markRaw(TextPinyin),
    textMint: markRaw(TextMint),
    textGradient: markRaw(TextGradient),
    textHigh: markRaw(TextHigh),
    iconsAntd: markRaw(IconsAntd),
    iconsIonicons5: markRaw(IconsIonicons5),
    iconsSelector: markRaw(IconsSelector),
    watermark: markRaw(Watermark),
    des: markRaw(Des),
    calendar: markRaw(Calendar),
    timeline: markRaw(Timeline),
    notice: markRaw(Notice),
    mapGaode: markRaw(MapGaode),
    mapBaidu: markRaw(MapBaidu),
    print: markRaw(Print),
    tagsView: markRaw(TagsView),
    moreComponents: markRaw(MoreComponents),
    waterfall: markRaw(Waterfall),
    importExcel: markRaw(ImportExcel),
    fingerPrintJs: markRaw(FingerPrintJs),
  };
